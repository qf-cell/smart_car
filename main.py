...
核心循环: step 1 网球 → step 2 沙包 → step 3 小熊
朝向序列: 0°(Y↑) → -90°(X→) → 0°(Y↑)
...
工作流程:
  ...
  4. 全部物体清空后返回起点, 同步检测信标 (AprilTag) 进行 PnP 重定位
...
def wait_reported_position(uart, state=None, timeout_ms=1200, label="下位机"):
    """等待下位机主动回传 @P#x#y!, 返回厘米坐标或 None。"""
    pos = read_position(uart, timeout_ms)
    if pos is None:
        print("[%s] 未收到 @P#x#y! 回传" % label)
        return None

    x, y = pos
    if state is not None:
        state.set_position(x, y)
    print("[%s] 回传位置: (%.1f, %.1f)" % (label, x, y))
    return pos
...
def _push_to_edge(uart, state, cur_x, cur_y,
    ...
    if pos is not None:
        cur_x, cur_y = pos
        print("[推送] 下位机回传位置 (%.1f, %.1f)" % (cur_x, cur_y))
    return cur_x, cur_y
...
def _push_bear_to_left_edge(uart, cur_x, cur_y, offset_y=OFFSET_CM):
    ...
    if pos is not None:
        ex, ey = pos
        print("[小熊推送] 下位机回传位置 (%.1f, %.1f)" % (ex, ey))
    return ex, ey
...
def main():
    ...
    while True:
        ...
        # ── 步骤 2: 90°(X←) → Y=170 → 扫描沙包 Y:170→70 ──
            ...
            pos = wait_reported_position(uart, state, 500, "到Y=170")
            if pos is not None:
                cmd_x, cmd_y = pos
            ...
            if cx is not None:
                ...
                _wait_frames(1500)
                cmd_x, cmd_y = 110, 70
                pos = wait_reported_position(uart, state, 500, "沙包补扫起点")
                if pos is not None:
                    cmd_x, cmd_y = pos
                ...
                if cx2 is not None:
                    ...
                    pos = read_position(uart, 500) or query_position(uart)
                    if pos is not None:
                        cur_x, cur_y = pos
        ...
        # ── 步骤 3: 0°(Y↑) → (210,70) → 扫描小熊 Y:70→170 ──
            ...
            if do_scan:
                ...
                _wait_frames(2000)
                cmd_x, cmd_y = 210, 70
                pos = wait_reported_position(uart, state, 500, "小熊扫描起点")
                if pos is not None:
                    cmd_x, cmd_y = pos
                ...
                if cx is not None:
                    ...
                    _wait_frames(1000)
                    cmd_x, cmd_y = 210, 170
                    pos = wait_reported_position(uart, state, 500, "小熊补扫起点")
                    if pos is not None:
                        cmd_x, cmd_y = pos
                    ...
                    cx2, cy2, px2, ax2, ay2, img2 = scan_bear_direct(
                        uart, 110, 70,
                        timeout=BEAR_SCAN_FRAMES)

                    if cx2 is not None:
                        print("[检测] X向补扫小熊! 图像(%d,%d) 像素:%d  位置(%.1f,%.1f)" %
                              (cx2, cy2, px2, ax2, ay2))
                        ...
                        if pos is not None:
                            ...
                            cur_x, cur_y = ax2 - OFFSET_CM, ay2
                            print("[小熊补扫] ⚠ 偏移后查位失败, 使用估算位置 (%.1f, %.1f)" %
                                  (cur_x, cur_y))

                        print("[小熊补扫] 平移至 Y=70 (保持 X=%.1f)" % cur_x)
                        ...
                        if pos is not None:
                            ...
                            cur_y = 70
                            print("[小熊补扫] ⚠ 到Y=70后查位失败, 使用估算位置 (%.1f, %.1f)" %
                                  (cur_x, cur_y))
